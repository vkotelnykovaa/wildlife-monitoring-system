              ...
              {loading ? (
                ...
                animals.map((animal, index) => {
                  const selectedAnimalIndex = animals.findIndex(
                    (item) => item.id === selectedAnimal?.id
                  );

                  const selectedRowStart =
                    selectedAnimalIndex === -1
                      ? -1
                      : Math.floor(selectedAnimalIndex / 3) * 3;

                  const selectedRowEnd = selectedRowStart + 2;

                  const shouldShowMapAfterThisAnimal =
                    selectedAnimal &&
                    index === Math.min(selectedRowEnd, animals.length - 1);
                        ...
                        <div className="col-span-full mt-4">
                          ...
                          {gpsData.length === 0 ? (
                            <div className="empty-state">
                              Даних для тварини {selectedAnimal.name} поки
                              немає.
                            </div>
                          ) : (
